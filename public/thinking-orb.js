(function () {
if (window.__thinkingOrbLoaded) return;
window.__thinkingOrbLoaded = true;

/* thinking-orb — web-component port of Jakub Antalik's `thinking-orbs`
   (github.com/Jakubantalik/thinking-orbs, MIT © Jakub Antalik).
   The geometry engine below is a faithful plain-JS port of src/engine/*;
   only the React binding is replaced with a custom element. */
...
function paintFrame(ctx, frame, dark, ink) {
  const tint = (w, alpha) => {
    ...
    const g = (dark ? 1 - v : v);
    if (ink) return `rgba(${ink[0]},${ink[1]},${ink[2]},${alpha * (dark ? g : 1 - g) })`;
    const c = Math.round(g * 255);
    return `rgba(${c},${c},${c},${alpha})`;
  };
  for (const l of frame.lines) {
    ctx.strokeStyle = tint(l.white, l.a ?? 1);
    ctx.lineWidth = l.w;
    ctx.beginPath();
  ...
  for (const d of frame.dots) {
    ctx.fillStyle = tint(d.white, d.a ?? 1);
    ctx.beginPath();
...
function frameGlobe(size, t, o) {
  const spin = 0.5, cx = size / 2, cy = size / 2;
...
function frameWeb(size, t, o) {
  ...
  const rs = radiusScale(size, o.rsPow ?? 0.6);
  const nodeN = o.nodeN ?? 30, thr = o.thr ?? 0.72, nodeR = o.nodeR ?? 1.4, nodeRDepth = o.nodeRDepth ?? 1.8;
  const nodes = [];
...
function resolvePreset(state, sizeKey) {
  ...
  const preset = PRESETS[mode][sizeKey] || PRESETS[mode][64];
...
class ThinkingOrbElement extends HTMLElement {
  static get observedAttributes() { return ['state', 'size', 'preset', 'theme', 'speed', 'ink']; }
  ...
  connectedCallback() {
    ...
    this._io = new IntersectionObserver(([e]) => {
      ...
      if (this._visible && document.visibilityState !== 'hidden') this.start(); else this.stop();
    }, { rootMargin: '80px' });
  ...
  attributeChangedCallback() { if (this.canvas) this.measure(); }

  measure() {
    const attrSize = parseFloat(this.getAttribute('size') || '');
    const box = this.getBoundingClientRect();
    const css = attrSize || Math.round(Math.min(box.width, box.height)) || 64;
    this.cssSize = css;
    const dpr = Math.min(2, window.devicePixelRatio || 1);
    this.canvas.width = Math.round(css * dpr);
    this.canvas.height = Math.round(css * dpr);
    if (attrSize) this.canvas.style.cssText = `display:block;width:${css}px;height:${css}px`;
    this.dpr = dpr;
    const state = this.getAttribute('state') || 'working';
    this.canvas.setAttribute('aria-label', this.getAttribute('aria-label') || LABELS[state] || 'Thinking…');
    const presetKey = this.getAttribute('preset') === '20' ? 20 : 64;
    this.resolved = resolvePreset(state, presetKey);
    const ink = this.getAttribute('ink');
    this.ink = ink ? ink.split(',').map(Number) : null;
    this.draw();
  }
  ...
  draw() {
    ...
    if (!ctx || !this.resolved) return;
    const size = this.cssSize;
    const mult = parseFloat(this.getAttribute('speed') || '1') || 1;
    const t = this.reduced ? 0.6 : (performance.now() / 1000) * this.resolved.speed * mult;
    ctx.setTransform(this.dpr, 0, 0, this.dpr, 0, 0);
    ctx.clearRect(0, 0, size, size);
    paintFrame(ctx, MODE_FRAMES[this.resolved.mode](size, t, this.resolved.opts), this.dark, this.ink);
  }

  start() {
    if (this._raf || this.reduced) { this.draw(); return; }
...
if (!customElements.get('thinking-orb')) customElements.define('thinking-orb', ThinkingOrbElement);
